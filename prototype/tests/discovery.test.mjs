...
test('India search retains and ranks a stronger global fallback',async()=>{
 ...
 const d=createDiscovery({env:{GEMINI_API_KEY:'test',SERPAPI_API_KEY:'test'},imageFetcher:async()=>({data:image.split(',')[1],mimeType:'image/png'}),fetcher:async(url,opts)=>{
...
test('slow fast-path providers get a longer Lens rescue and return visual leads',async()=>{
 let lensCalls=0;const d=createDiscovery({env:{SERPAPI_API_KEY:'test',PUBLIC_BASE_URL:'https://preview.example.com'},fetcher:async url=>{
...
test('India provider outage returns domestic retailer searches instead of US products',async()=>{
 const d=createDiscovery({env:{SERPAPI_API_KEY:'test'},fetcher:async()=>{throw Error('timeout')}});const out=await d.search({attributes:attr,market:'in'});assert.equal(out.trace.retailerShortcuts,true);assert.equal(out.results.length,3);assert.ok(out.results.every(p=>p.market==='in'&&['www.myntra.com','www.amazon.in','www.flipkart.com'].includes(new URL(p.url).hostname)));
});
...
test('auto mode records a result and replay mode uses it without provider calls',async()=>{
 ...
 const record=createDiscovery({env:{SERPAPI_API_KEY:'test',DISCOVERY_DATA_MODE:'auto'},replayStore,fetcher:async url=>{const q=new URL(url).searchParams.get('q')||'';return response({images_results:q.includes('site:')?results:[global]});}});
